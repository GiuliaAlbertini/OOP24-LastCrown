...
/**
 * A standard implementation of AudioEngine interface.
 */
public final class AudioEngineImpl implements AudioEngine {
    private static final Logger LOG = Logger.getLogger(AudioEngineImpl.class.getName());
    ...
    @Override
    public synchronized void playSoundTrack(final SoundTrack soundTrack) {
    ...
    @Override
    public synchronized void playEffect(final SoundEffect soundEffect) {
        final CompletableFuture<Void> task = CompletableFuture.runAsync(() -> {
            try {
                stopCurrent();
    ...
    private void fadeOutAndStop(final Clip clip) {
